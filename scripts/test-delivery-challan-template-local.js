#!/usr/bin/env node
/**
 * Test the Delivery Challan classic template locally without deployment.
 *
 * Usage:
 *   node scripts/test-delivery-challan-template-local.js [--html]
 *
 * Output:
 *   test-pdfs/delivery-challan-classic-preview.html
 */
...
function getSampleChallan() {
    return {
        challanNumber: 'DC-2026-001',
        challanDate: '2026-02-16',
...
async function main() {
    ...
    console.log('\nRendering Delivery Challan (classic) with sample data...\n');

    const challan = getSampleChallan();
    let html;
    try {
        html = await invoicePdfService.renderDeliveryChallanHtml(challan, 'classic');
    } catch (err) {
        console.error('Failed to render HTML:', err.message);
        process.exit(1);
    }

    const htmlPath = path.join(outputDir, 'delivery-challan-classic-preview.html');
    ...
    if (!htmlOnly) {
        try {
            ...
            const pdfPath = path.join(outputDir, 'delivery-challan-classic-preview.pdf');
